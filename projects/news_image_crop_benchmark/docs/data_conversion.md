...
- 保留同一图片对应的多个标题样本，同时使用原图内容 SHA-256 防止 URL 别名导致跨数据划分泄漏。
...
- 供源数据审计使用的 URL 分桶；该分桶不是最终训练 split。
...
以较小的 record batch 扫描 `OriginalImageUrl` 和 `OriginalImageBytes`。对于每个图片内容 SHA-256 的首次出现：
...
不同 URL 只要图片字节完全相同，也复用同一个图片资产。源文件只有一个 3.1 GB 的 row group，因此转换器不得使用包含 `OriginalImageBytes` 的 `read_table` 一次性读取整表。

### 3. 去重并展开比例

按照完全相同的 `(OriginalImageSha256, normalized GemTitle)` 组合去重。11,390 条源数据中共有 9,770 个唯一可信组合。每个组合分别生成目标比例为 `1.0`、`1.91`、`1.77` 和 `1.59` 的四个独立任务，共得到 39,080 条数据。

仅根据图片内容 SHA-256 分配一次数据划分；相同图片内容对应的所有 URL、标题和比例都必须位于同一个 split。最终训练 ID 由图片 SHA-256、规范化标题和目标比例共同生成。
...
- 每个 sample ID 都唯一。
- train/validation/test 中的图片 SHA-256 集合两两不相交。
